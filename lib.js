...
_.readdirsByTree = function(dir){
  var files = fs.readdirSync(dir);
  return files.map(function(file) {
    var info = {
      path: file,
      realpath: path.join(dir, file)
    };

    info.type = fs.statSync(info.realpath).isFile() ? 'file' : 'dir';

    if (info.type == 'dir') {
      info.children = _.readdirsByTree(info.realpath);
    } 
